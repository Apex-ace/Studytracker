  ...
  const writes = CHAPTER_PLAN_STAGES.map(async (stage) => {
    ...
    if (!plannedDate) {
      ...
      if (
        ...
        existingData?.assignedByAdmin !== true
      ) {
        await deleteDoc(ref);
      }
      return;
    ...
    const status = actualDate
      ...
      : existingData?.status === "COMPLETED"
        ? autoPlanStatus(plannedDate, "")
        : existingData?.status || autoPlanStatus(plannedDate, "");

    if (existingData?.assignedByAdmin === true) {
      await updateDoc(ref, {
        status,
    ...
    await setDoc(
      ...
      {
        userId: uid,
        title: existingData?.title || `${stage.label}: ${chapter.title || "Chapter"}`,
        subject: prettySubjectName(subjectSlug),
        activityType: existingData?.activityType || stage.activityType,
        scheduledDate: plannedDate,
        startTime,
        endTime,
        status,
        notes: existingData?.notes ?? `Automatically linked to ${stage.label} in chapter progress.`,
        autoGenerated: true,
        source: "CHAPTER_PLAN",
...
export async function addChapterStageActivity(uid, chapter, subjectSlug, stageKey, values = {}) {
  const stage = CHAPTER_PLAN_STAGES.find((item) => item.key === stageKey);
  if (!uid || !chapter?.id || !stage) throw new Error("Invalid chapter stage activity.");

  const scheduledDate = String(values.scheduledDate || "").trim();
  if (!scheduledDate) throw new Error("Activity date is required.");

  const allowedStatuses = ["NOT_STARTED", "IN_PROGRESS", "COMPLETED", "MISSED"];
  const requestedStatus = String(values.status || "NOT_STARTED");
  const requestedSafeStatus = allowedStatuses.includes(requestedStatus) ? requestedStatus : "NOT_STARTED";
  const existingActualDate = String(values.existingActualDate || "").trim();
  const actualDate = requestedSafeStatus === "COMPLETED" ? scheduledDate : existingActualDate;
  const status = actualDate ? "COMPLETED" : requestedSafeStatus;
  ...
  if (actualDate) progressValues[stage.actualField] = actualDate;
  ...
  batch.set(
    ...
    {
      userId: uid,
      title: String(values.title || "").trim() || `${stage.label}: ${chapter.title || "Chapter"}`,
      subject: prettySubjectName(subjectSlug),
      subjectSlug,
      chapterId: chapter.id,
      chapterTitle: chapter.title || "",
